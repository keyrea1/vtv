...
export default class StaticTActionsTruck extends JetView {
	config () {
		...
		function setArchive (id) {
			...
			var obj = $$(datatable_name).getItem(id);
			console.log(obj);
			webix.ajax().post(
				ip,
				{
		...
		if (access.change_wagon_number)
			wagonNumberField = {
				...
				adjust: "header", editor: "text", format: function (value) {return webix.i18n.intFormat(value);},
		...
		return {
			...
			columns: [
				...
				},
				{
					...
					editor: "myeditor", adjust: true, liveEdit: true
				},
				{
					id: "reciever", header: _("reciever"),
					editor: "myeditor", adjust: true
				},
				{
			...
			on: {
				"onAfterEditStop": function (state, editor, ignoreUpdate) {
					var row = editor.row;
					try {
						var obj = $$(datatable_name).getItem(row);
						console.log(obj);
					}
					catch (e) {
						console.log(e);
				...
				"onItemClick": function (id, e, trg) {
					if (id.column === "save") {
						...
						//Проверка не происходит ли сейчас взвешивание
						//if ($$("taraControl").isEnabled()) {
							webix.modalbox({
							...
							return false;
						//}
						//else webix.message({text: _("Finish current weighing first"), type: "error", expire: 5000});
					}
				}
			},
	...
	init (grid) {
		...
		function getOperationTable () {
			...
			webix.ajax().post(
				ip, {"method": "getOperationTable", "user": User, "params": {"type": "static.truck"}},
				function (text, xml, xhr) {
					var data = JSON.parse(text);
					console.log(data);
					if (data.method === _methodName) {
